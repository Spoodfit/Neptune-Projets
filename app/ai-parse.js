...
function proposeAiAction(text) {
  ...
  if (createIntent) {
    ...
    if (!rawName || rawName.length < 3) return { type: 'clarify', message: 'Donne-moi simplement le nom du projet. Exemple : « Crée un nouveau projet : Refonte du site ». ' };
    ...
    const draftProject = {
      ...
      summary: 'Projet créé depuis Neptune AI. La structure se précisera au fil des prochaines actions et décisions.',
      stages: [
      ...
      blockers: [],
      roles: [{ id: 'lead', name: 'À assigner', role: 'Chef de projet', initials: '?', tone: 'slate', state: 'good', message: 'Le responsable et les premières actions restent à préciser.', tasks: [] }],
    };
    return { type: 'project-create', project: draftProject, message: `Je crée <strong>${escapeHtml(rawName)}</strong> dans l’espace <strong>${escapeHtml(state.workspace.name)}</strong>, avec un cadrage minimal et aucune donnée inutile.` };
  }
  const project = findProjectFromText(text) || (activeProjectId ? state.projects.find((p) => p.id === activeProjectId) : null);
  if (!project) return { type: 'clarify', message: 'Je vois la mise à jour, mais pas le projet concerné. Donne-moi simplement son nom et je rattache le changement au bon endroit.' };
  ...
  let targetStatus = wantsBlocked ? 'blocked' : wantsDone ? 'done' : wantsProgress ? 'progress' : null;
  if (taskMatch && targetStatus) {
    return {
      type: 'task-update', project, role: taskMatch.role, task: taskMatch.task, targetStatus,
      message: `Je comprends que <strong>${escapeHtml(taskMatch.task.title)}</strong> dans <strong>${escapeHtml(project.name)}</strong> passe en <strong>${escapeHtml(taskMeta(targetStatus).label.toLowerCase())}</strong>.`,
    };
  }
  ...
  if (wantsDone && /(android|play console)/.test(normalized) && project.id === 'connexio') {
    ...
    if (role && task) return { type: 'task-update', project, role, task, targetStatus: 'done', message: `Je comprends que <strong>${escapeHtml(task.title)}</strong> est terminé dans <strong>${escapeHtml(project.name)}</strong>.` };
  }

  return {
    type: 'project-note', project,
    message: `J’ai rattaché cette mise à jour à <strong>${escapeHtml(project.name)}</strong>. Je n’ai pas assez de certitude pour modifier une tâche automatiquement. Je peux l’ajouter au journal du projet sans changer son état.`,
  };
}
